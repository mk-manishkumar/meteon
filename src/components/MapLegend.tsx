...
export default function MapLegend({ mapType }: Props) {
  ...
  const maxValue = lastStop.value;

  const gradientStops = data.stops.map((stop) => `${stop.color} ${(stop.value / maxValue) * 100}%`).join(", ");

  return (
    <div className="absolute top-4 right-4 z-1000 w-48 xs:w-96 rounded-xl border border-accent/70 bg-background/50 p-4 shadow-lg flex flex-col gap-4">
